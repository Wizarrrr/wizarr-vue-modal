...
describe("Init", () => {
  it("Run without container, must throw the error", async function () {
    await expect(() => pushModal(ModalTitle)).rejects.toThrowError(
      ModalError.NotInitialized()
    );
  });
  ...
  it("provide props", async () => {
    ...
    expect(wrapper.text()).toBe("Wizarr 22");
  });
  ...
  it("close by id", async () => {
    ...
    expect(modalQueue.value.map((item) => item.id)).toEqual([
      modal1.id,
      modal3.id,
    ]);
  });
